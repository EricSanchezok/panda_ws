...
def main():
    ...
    target_pose.header.frame_id = panda.arm.get_planning_frame()
    target_pose.pose.position.x = random.uniform(0.3, 0.7)
    target_pose.pose.position.y = random.uniform(-0.3, 0.3)
    target_pose.pose.position.z = random.uniform(0.3, 0.7)
    ...
    # 设置机械臂终端运动的目标位姿
    panda.arm.set_pose_target(target_pose, panda.end_effector_link)
