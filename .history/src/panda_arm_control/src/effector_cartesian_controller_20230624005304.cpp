...
bool EffectorCartesianController::init(hardware_interface::RobotHW* robot_hardware,
  ...
  model_handle_ = robot_hardware->get<franka_hw::FrankaModelInterface>()->getHandle(arm_id + "_model");
  if (model_handle_ == nullptr) {
    ROS_ERROR_STREAM("EffectorCartesianController: Could not get handle for arm_id: " << arm_id);
    return false;
  }

  state_handle_ = robot_hardware->get<franka_hw::FrankaStateInterface>()->getHandle(arm_id + "_robot");
  if (state_handle_ == nullptr) {
    ROS_ERROR_STREAM("EffectorCartesianController: Could not get handle for arm_id: " << arm_id);
    return false;
  }
...
void EffectorCartesianController::update(const ros::Time& /* time */,
    ...
    std::vector<double> robot_jointPositions = state_handle_->getRobotJointPositions();
    std::vector<double> robot_jointVelocities = state_handle_->getRobotJointVelocities();
