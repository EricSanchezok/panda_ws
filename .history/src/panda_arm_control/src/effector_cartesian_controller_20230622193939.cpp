...
void EffectorCartesianController::update(const ros::Time& /* time */,
    ...
    # 打印当前索引占总长度的比例
    std::string message = "当前轨迹段索引占总长度的比例：";
    message += std::to_string(segment_index);
    message += "/";
    message += std::to_string(trajectory_.points.size());
    ...
    for (size_t i = 0; i < positions.size(); ++i)
    {
      ...
      // 根据关节速度和时间偏移计算当前时间点的关节速度
      double current_velocity = velocity * segment_completion;
