...
bool EffectorCartesianController::init(hardware_interface::RobotHW* robot_hardware,
  ...
  ros::Subscriber sub_effector_control_param_ = node_handle.subscribe(
    "effector_control_param", 20, &EffectorCartesianController::controlParamCallback, this,
    ros::TransportHints().reliable().tcpNoDelay());
  ros::Subscriber sub = nh.subscribe<panda_arm_control::effector_control_param>("effector_control_param_node",20,&EffectorCartesianController::controlParamCallback);

  velocity_joint_interface_ = robot_hardware->get<hardware_interface::VelocityJointInterface>();
  if (velocity_joint_interface_ == nullptr) {
    ROS_ERROR("EffectorCartesianController: 从硬件中获取速度关节接口时出错！");
    return false;
  }
  ...
  if (!node_handle.getParam("arm_id", arm_id)) {
    ROS_ERROR("EffectorCartesianController: 无法获取参数 arm_id");
    return false;
  }

  std::vector<std::string> joint_names;
  if (!node_handle.getParam("joint_names", joint_names)) {
    ROS_ERROR("EffectorCartesianController: 无法解析关节名称");
  }
