...
bool EffectorCartesianController::init(hardware_interface::RobotHW* robot_hardware,
  ...
  model_handle_ = robot_hardware->get<franka_hw::FrankaModelInterface>()->getHandle("panda");
  if (model_handle_ == nullptr) {
    ROS_ERROR("EffectorCartesianController: Error getting model handle from hardware!");
    return false;
  }

  state_handle_ = robot_hardware->get<franka_hw::FrankaStateInterface>()->getHandle("panda");
  if (state_handle_ == nullptr) {
    ROS_ERROR("EffectorCartesianController: Error getting state handle from hardware!");
    return false;
  }
