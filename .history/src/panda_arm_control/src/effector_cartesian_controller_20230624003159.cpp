...
void EffectorCartesianController::update(const ros::Time& /* time */,
    ...
    // 根据比例插值计算当前时间点的关节位置
    std::vector<double> current_velocitys;
    for (size_t i = 0; i < velocitys.size(); ++i)
    {
      double velocity = velocitys[i];
      double velocity = velocities[i];

      // 根据关节速度和时间偏移计算当前时间点的关节位置
      double delta_velocity = velocity * segment_completion * period.toSec();
      double current_velocity = velocity + delta_velocity;
      current_velocitys.push_back(current_velocity);
    }

    for (size_t i = 0; i < 7; ++i) {
      velocity_joint_handles_[i].setCommand(current_velocitys[i]);
    }
