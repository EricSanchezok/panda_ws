...
void EffectorCartesianController::update(const ros::Time& /* time */,
  ...
  if (trajectory_elapsed_time_ >= joint_trajectory.points.back().time_from_start)
  {
  ...
  while (segment_index < joint_trajectory.points.size() &&
         trajectory_elapsed_time_ >= joint_trajectory.points[segment_index].time_from_start)
  {
    segment_index++;
  }

  // 获取当前轨迹段的关节位置和速度
  const trajectory_msgs::JointTrajectoryPoint& current_segment = joint_trajectory.points[segment_index - 1];
  const trajectory_msgs::JointTrajectoryPoint& next_segment = joint_trajectory.points[segment_index];
