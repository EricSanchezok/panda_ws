...
class mediaPipeHand:
    ...
    def get_world_points(self, server):

        server.results = self.hands.process(cv2.cvtColor(server.show_color_image, cv2.COLOR_BGR2RGB))

        if server.results.multi_hand_landmarks and server.draw_finger:
            for handLms in server.results.multi_hand_landmarks:
                self.mpDraw.draw_landmarks(server.show_color_image, handLms, self.mpHands.HAND_CONNECTIONS, self.handLmsStyle, self.handConStyle)

        if server.results.multi_hand_world_landmarks:
            points = []
            for handLms in server.results.multi_hand_world_landmarks:
            ...
            server.points = np.array(points).reshape(1, -1)
        else:
            server.points = None
            server.results = None