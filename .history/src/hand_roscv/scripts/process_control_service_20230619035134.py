...
def doReq(req):
    ...
    if req.request_num == 0:
        ...
        if server.task_distributor.srv_value is None:
            return control_srvResponse(value)
        
        
        value = []
        value.append(server.task_distributor.work_mode)
        value.append(server.task_distributor.srv_value[0])
        value.append(server.task_distributor.srv_value[1])
        value.append(server.task_distributor.srv_value[2])
        msg = control_srvResponse(value)
