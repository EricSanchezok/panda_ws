...
class server:
    ...
    def server_process_frame(self):
        ...
        if self.fliter_point is not None:
            value = []
            value.append(self.fliter_point[0])
            self.vs2Dvalue.draw_2Dvalue(value)
...
def doReq(req):
    ...
    msg.task = server.task_distributor.work_mode
    msg.vector0 = server.task_distributor.srv_value[0]
    msg.vector1 = server.task_distributor.srv_value[1]
    msg.vector2 = server.task_distributor.srv_value[2]
    ...
    if req.request_num == 0:
        ...
        return [server.task_distributor.work_mode, server.task_distributor.srv_value[0], server.task_distributor.srv_value[1], server.task_distributor.srv_value[2]]
