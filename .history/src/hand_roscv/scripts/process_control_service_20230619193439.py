...
class server:
    ...
    def server_process_frame_savemode(self):
        ...
        self.handLandmarks_points, self.result = self.hand_detector.get_world_points(self.show_color_image, draw_finger=self.draw_finger)

        pg.cal_finger_angle(self)
        pg.get_pred(self)

        #左右翻转
        flip_show_color_image = cv2.flip(server.ori_color_image, 1)
        self.handLandmarks_points, self.result = self.flip_hand_detector.get_world_points(self.flip_show_color_image, draw_finger=self.draw_finger)

        # 拼接图像
        show_image = np.hstack((self.show_color_image, flip_show_color_image))
        # 图像尺寸减半
