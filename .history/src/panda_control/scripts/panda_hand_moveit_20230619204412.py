    ...
    while not rospy.is_shutdown():
        ...
        if panda.if_init_ready_pose == False:
            ...
            if panda.control_value.task == "position" and is_zero(panda.control_value) == False:
                ...
                if fraction == 1.0:
                    print("调整前规划点数量:", len(plan.joint_trajectory.points))
                    plan = panda.arm.retime_trajectory(panda.robot.get_current_state(), plan, 1.0)
                    print("调整后规划点数量:", len(plan.joint_trajectory.points))
