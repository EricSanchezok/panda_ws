    ...
    while not rospy.is_shutdown():
        ...
        if panda.if_init == False:
            ...
            if panda.control_value.task == "position":
                ...
                distance = pow((panda.control_value.vector0), 2) + pow((panda.control_value.vector1), 2) + pow((panda.control_value.vector2), 2)
                if distance < 0.01:
                    continue
                ...
                # 计算当前位置与目标位置之间的欧式距离
                distance = pow((target_pose.position.x - start_pose.position.x), 2) + pow((target_pose.position.y - start_pose.position.y), 2) + pow((target_pose.position.z - start_pose.position.z), 2)

                if distance < 0.01:
                    print("当前距离:", distance, " 过小，不进行运动")
                    continue
