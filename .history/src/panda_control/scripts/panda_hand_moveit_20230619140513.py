...
def main():
    ...
    while not rospy.is_shutdown():
        ...
        if panda.if_init == False:
            ...
            return_value = panda.clt.call(num)

            panda.control_value.task = return_value.task

            if return_value.vector0 >= 10e-3:
                panda.control_value.vector0 += return_value.vector0
            if return_value.vector1 >= 10e-3:
                panda.control_value.vector1 += return_value.vector1
            if return_value.vector2 >= 10e-3:
                panda.control_value.vector2 += return_value.vector2
            ...
            if panda.control_value.task == "position":
                ...
                distance = math.sqrt(pow((panda.control_value.vector0), 2) + pow((panda.control_value.vector1), 2) + pow((panda.control_value.vector2), 2))
                print("当前距离:", distance)
                if distance < 0.1:
                    continue
                ...
                target_pose.position.x = panda.start_pose.position.x + panda.control_value.vector0
                target_pose.position.y = panda.start_pose.position.y + panda.control_value.vector1
                target_pose.position.z = panda.start_pose.position.z + panda.control_value.vector2

                panda.control_value.vector0 = 0
                panda.control_value.vector1 = 0
                panda.control_value.vector2 = 0
